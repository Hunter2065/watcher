...
describe Watcher do
  ...
  describe '#methods' do
    context ':start_directory_monitoring' do
      context 'when action is :create' do
        it 'should be successful for DIRECTORY' do
          ...
          expect_any_instance_of(Filewatcher).to receive(:watch).and_yield(
            'root/path/to/parent/new_folder',
            :created
          )
      ...
      context 'when action is :deleted' do
        context 'for DIRECTORY' do
          it 'should be successful for DIRECTORY' do
            # triggered event :deleted
            expect_any_instance_of(Filewatcher).to receive(:watch).and_yield(
              'root/path/to/parent/test_folder',
              :deleted
            )

            # get relative path
            #send to server
            expect(ServerClient).to receive(:delete).with(
              relative_path: 'parent/test_folder'
            ).and_return(OpenStruct.new(code: 200))
